...
function M.get_send_text()
  ...
  if is_empty(cur) then
    return nil, cursor_line, cursor_line
  end
  if is_comment(cur) then
    return cur, cursor_line, cursor_line
  end
...
function M.debug()
  ...
  if #lines == 0 then
    vim.notify("[replent] Buffer is empty")
    return
  end
  ...
  vim.notify(
    string.format(
      "[replent/matlab] line %d: %q\n  is_block_start=%s  is_block_end=%s  comment=%s",
      cursor_line,
      cur or "",
  ...
  local s, e = M.get_block_range(bufnr, cursor_line)
  vim.notify(string.format("[replent/matlab] block range: %d → %d", s, e))
end
